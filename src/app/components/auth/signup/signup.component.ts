...
export class SignupComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.animateSignupForm();
  }

  private animateSignupForm(): void {
    const tl = gsap.timeline();
    
    tl.fromTo(this.signupForm.nativeElement,
      { opacity: 0, y: 50 },
      { opacity: 1, y: 0, duration: 0.8, ease: "power2.out" }
    )
    .fromTo(this.nameField.nativeElement,
      { opacity: 0, x: -30 },
    ...
    if (this.signupFormGroup.valid && !this.isLoading) {
      ...
      try {
        ...
        gsap.to(this.submitButton.nativeElement, {
          ...
          repeat: 1,
          onComplete: () => {
            this.snackBar.open('Account created successfully!', 'Welcome to CloudRunr!', {
              duration: 3000,
        ...
        this.snackBar.open('Signup failed. Please try again.', 'Error', {
          duration: 4000,
  ...
  private markFormGroupTouched(): void {
    Object.keys(this.signupFormGroup.controls).forEach(key => {
      const control = this.signupFormGroup.get(key);
